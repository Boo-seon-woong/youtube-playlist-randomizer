...
- **가사 플로팅 창** — 재생 바의 책 버튼으로 Lyrs 스타일의 항상 위 가사 창을 열 수 있음. 현재 곡을 자동 인식해 **ALSong DB의 한글·번역 가사를 우선** 표시하고, 한글 가사가 없을 때만 LRCLIB 원어 가사로 보완하며 "한글 번역 없음 · 원어 가사"를 표시. 창은 드래그로 이동·위치 저장하고, 가사 창의 설정 버튼은 잘림 없이 조절할 수 있는 **별도 설정 창**을 열어 가로·세로 크기, 배경 투명도, 글씨 크기, 재생바·곡 정보·앨범 이미지·상태 문구, 재생 컨트롤과 이전/일시정지/다음 버튼, 항상 위 표시를 각각 조절할 수 있음
- **패널 접기·폭 조절** — 왼쪽 '내 플레이리스트'와 오른쪽 '재생 대기열'은 패널 사이 경계를 드래그해 폭을 바꾸고, 경계 가운데의 ‹ › 버튼으로 접었다 펼칠 수 있음. 가운데 영상도 하단 바의 **최소화** 버튼으로 화면 구성에서 숨겨 목록을 넓게 볼 수 있음 (재생은 그대로 이어지고, 폭·접힘·최소화 상태는 자동 저장)
...
5. 하단 재생 바의 **가사 버튼(책 아이콘)**을 누르면 현재 곡의 가사 플로팅 창이 열립니다. 창을 드래그해 위치를 옮길 수 있으며, 설정 버튼에서 창 크기·투명도·글씨 크기·재생바·재생 제어 버튼 등의 표시 여부를 조절할 수 있습니다. 검색 버튼으로 다른 ALSong/LRCLIB 결과를 선택할 수 있고, 가사가 원어로만 제공되면 창 하단에 안내가 표시됩니다.
