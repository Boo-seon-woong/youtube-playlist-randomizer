...
- **폴더 정리** — Windows 탐색기처럼 재생목록을 드래그해 다른 재생목록 위에 놓으면 폴더로 묶임. 폴더끼리 겹치면 병합, 빈 공간에 놓으면 폴더 밖으로, `+ 새 폴더` 버튼으로 빈 폴더 생성
...
- **몰입 모드** — 전체화면 시청이 곡 전환 시에도 끊기지 않음
...
3. 왼쪽 목록 정리:
   ...
   - 폴더 위에 놓으면 그 폴더에 추가되고, 폴더끼리 겹치면 병합됩니다. 목록의 빈 공간에 놓으면 폴더 밖으로 나옵니다.
   - 폴더를 클릭하면 접거나 펼칩니다. 연필 버튼으로 이름·링크를 수정합니다.
   - 폴더를 삭제해도 안의 재생목록은 지워지지 않고 밖으로 이동합니다.
4. 오른쪽 대기열에서:
   - 곡을 클릭하면 해당 곡으로 이동합니다.
   - 휴지통 버튼으로 이번 감상에서 곡을 뺄 수 있습니다 (저장된 재생목록은 바뀌지 않음).
   - 하단 **이전 / 다음 / 셔플** 버튼으로 재생을 제어합니다.
5. 전체화면 감상 중 곡이 전환되면 몰입 모드로 이어집니다. **Esc** 또는 우상단 버튼으로 해제합니다.

## 알아두기

- "직접 재생" 배지가 붙은 곡은 실제 유튜브 페이지로 재생되므로 광고가 잠깐(무음) 스칠 수 있습니다.
