...
- **영상 최소화** (`layout.centerMin`, 하단 바 최소화 버튼 / 미니 플레이어의 복원 버튼):
  `body.center-min`에서 `#content`를 `position: fixed` 320×180(+복원 버튼 헤더 띠 26px) 우하단 미니 플레이어로 띄우고
  대기열(사이드바가 접혔으면 사이드바)이 `flex: 1`로 가운데를 채운다. `#resize-right`는 숨겨
  사이드바|대기열 사이 핸들 하나만 남는다. **`display: none`으로 숨기지 않는 이유**: 직접 재생
  webview는 레이아웃에서 빠지면 끊길 수 있다 — 작게 유지하면 임베드·폴백 모두 재생이 이어진다.
  곡 구성·검색 오버레이는 플레이어 영역 안에 그려지므로 열 때 `setCenterMin(false)`로 자동 복원하고,
  몰입 모드 진입도 복원한다.
