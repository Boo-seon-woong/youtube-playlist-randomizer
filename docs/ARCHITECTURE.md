...
- **광고 음소거 지연 제거**: 100ms 인터벌만으로는 광고 시작 후 최대 0.1초 소리가 샌다.
  `#movie_player`의 class 변화를 `MutationObserver`로 잡아 `ad-interrupting`/`ad-showing`이 붙는 즉시
  `video.muted = true`로 만든다(인터벌은 백업 겸 옵저버 설치 재시도). 임베드 경로는 주입이 불가능해
  이 처리가 없고, 대신 광고 도메인 차단이 임베드 광고를 막는다.
...
- **스크럽 커버**(`#player-cover`): 재생바를 누르는 순간부터 이동 1.1초 후까지 플레이어를 앨범 아트로
  덮는다. `controls: 0`으로도 임베드는 **`seekTo()` 순간 제목줄·'동영상 더보기' 오버레이·일시정지
  베젤을 수백 ms 띄운다**(사용자가 놓는 순간 캡처한 스크린샷이 증거 — CDP 캡처는 300ms 이상 걸려 놓친다).
  교차 출처라 CSS로 지울 수 없으므로 그 구간을 가린다. 검증(실제 마우스 입력): 누른 직후·드래그 중·
  놓은 직후 커버 표시, 1.5초 뒤 해제.
