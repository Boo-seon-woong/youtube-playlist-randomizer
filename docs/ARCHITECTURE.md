...
- **체감 볼륨 커브**: 출력 = (슬라이더/100)² × 100 (제곱 커브). 1은 충분히 작고 100은 충분히 크게 —
  선형이면 저볼륨이 크게 들려 차이가 좁게 느껴진다(사용자 요청). UI·저장값은 선형 그대로이며, 임베드는
  커브 출력이 0으로 반올림되는 구간(<0.5)을 mute()로 처리한다(unMute가 볼륨 5로 되살리는 quirk 회피).
