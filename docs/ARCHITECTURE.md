...
- **Alt+Q/E 길게 누르기**: 전역 단축키는 키를 뗀 순간을 모르지만, 누르고 있으면 OS 자동 반복으로 콜백이
  ...
  반복이 없으면 이전/다음 곡으로 확정. 대기 시간은 처음 550ms에서 시작해 **첫 반복이 도착하는 간격을
  관측하면 그 값+60ms(200~700ms)로 줄인다** — Windows 키 반복 지연(기본 약 500ms, 설정에 따라 250ms~1초)보다
  짧게 잡으면 첫 반복이 오기 전에 곡이 넘어가고(실측 신고), 길게 잡으면 단발 입력이 늦어지므로 PC마다 맞춘다.
