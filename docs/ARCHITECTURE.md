...
- **전역 단축키(Alt+1 / Alt+2)**: `globalShortcut`으로 등록한 OS 핫키. Alt+2(가사 창 표시/숨김)는
  `showInactive`/`hide`라 **포커스를 건드리지 않는다**. Alt+1(잠금 토글)은 반대로 **조작 주체까지
  옮긴다** — 전체화면 게임은 마우스 커서를 잡고 있어서 잠금만 풀어도 포커스가 게임에 있는 한 커서가
