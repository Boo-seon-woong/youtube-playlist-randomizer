...
- **전역 단축키(Alt+Y / Alt+U)**: `globalShortcut`으로 등록한 OS 핫키라 게임이나 작업 중인 프로그램의
  **포커스를 뺏지 않는다**(우리 창을 활성화하지 않고, 표시도 `showInactive`, 잠금도
  `setIgnoreMouseEvents`라 포커스와 무관). Alt+Y는 잠금 토글, Alt+U는 가사 창 표시/숨김.
