...
- 렌더러가 넘기는 제목·아티스트는 유튜브 **영상 제목과 채널명**이다(`[MV] IU(아이유) _ Good Day(좋은 날)`,
  ...
  `buildLyricQueries`가 `[MV]`·`Official MV`·`(가사/Lyrics)`·`(Feat. …)` 같은 태그를 걷어내고, 따옴표
  제목(`'Dynamite'`, `「アイドル」`)이나 구분자(`-` `_` `|` `:`)로 아티스트/제목을 나눈 뒤 괄호 병기
  (`Good Day(좋은 날)` → `좋은 날`, `Good Day`; 한글 우선)와 채널명(`- Topic`, `Official`, `VEVO` 제거)에서
  후보를 뽑아 **제목×아티스트 → 뒤집은 조합(Title - Artist 순서 대비) → 제목 단독** 순으로 첫 결과가
  나올 때까지 검색한다. 아티스트 이름을 제목으로 검색하지는 않는다(엉뚱한 곡만 걸림). 수동 검색
  (`lyrics:search`)도 같은 정제를 거친다.
