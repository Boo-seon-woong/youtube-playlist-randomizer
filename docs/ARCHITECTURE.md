...
| `lyrics.html`, `lyrics.css`, `lyrics.js` | 투명 플로팅 가사 창 — 왼쪽 앨범 아트(곡 정보 오버레이·재생바·재생 컨트롤) + 오른쪽 이전/현재/다음 블록(줄마다 칩 배경), 마우스를 올리면 검색·재시도·설정·숨기기 버튼, 드래그 위치 저장 |
...
- **창 디자인**: 카드 패널 없이 완전 투명 배경. 왼쪽에 앨범 아트 정사각형(하단 그라디언트 위에 제목·아티스트,
  그 아래 옵션인 재생바·이전/일시정지/다음), 오른쪽에 이전·현재·다음 블록을 왼쪽 정렬로 세로 나열한다.
...
- 창 설정 UI는 메인 창 **디자인 설정 패널을 아래로 스크롤하면 나오는 "플로팅 가사 창" 섹션**이다
  ...
  배경 불투명도·글씨 크기·곡 정보/앨범 이미지/상태 문구, 재생바와 재생 컨트롤·이전/일시정지/다음 버튼,
  항상 위 표시를 각각 토글할 수 있다. 가사 창의 컨트롤은 IPC를 통해 메인 재생 대기열의
  이전/다음/일시정지 동작을 호출한다.
