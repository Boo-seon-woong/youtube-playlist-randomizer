...
| `lyrics.html`, `lyrics.css`, `lyrics.js` | 투명 플로팅 가사 창 — 왼쪽 앨범 이미지/미러 영상(곡 정보 오버레이·재생바·재생/볼륨 컨트롤) + 오른쪽 이전/현재/다음 블록(줄마다 칩 배경), 마우스를 올리면 설정(톱니) 버튼만(검색·재시도·숨기기는 제거 — 가사 검색은 메인 창, 표시 토글은 Alt+3/설정), 드래그 위치 저장 |
...
- **광고 도메인 차단은 메인 창 요청에만 적용한다.** `onBeforeRequest`가 `details.webContentsId`가
  ...
  전면 차단 화면이 뜨면 주입 스크립트가 `__ymp_enforced:1`을 보고하고, 호스트는 감지를 피해 다니는 대신
  **그 세션 동안 광고 차단을 끄고**(`adblock:disable`) 한 번만 다시 시도하며, 그래도 막히면 큐가 멈추지
  않도록 다음 곡으로 넘긴다(`handleAdBlockEnforcement`). 대신 차단이 꺼진 뒤로는 임베드 광고가 그대로
  재생된다 — 크로스 오리진이라 앱이 무음·스킵을 걸 수 없다.
...
- **체감 볼륨 커브**: 출력 = 100 × 10^((슬라이더−100)/50) — dB 선형(로그) 매핑, 전 구간 40dB.
  제곱 커브는 저볼륨이 0으로 반올림돼 슬라이더 1~7이 통째로 무음이 되는 데드존을 만들었다(실측 신고) —
  로그 커브는 1부터 출력 ≥1이라 바로 들린다. UI·저장값은 선형 그대로. 임베드(IFrame API)는 정수 볼륨만
  받아 인접한 낮은 정수끼리는 같은 출력으로 반올림될 수 있고(물리적 한계), 직접 재생은 float라 모든
  단계가 구분된다. 무음은 슬라이더 0에서만(mute(); unMute가 볼륨 5로 되살리는 quirk 회피).
...
- **직접 재생(폴백)**: dom-ready와 볼륨 변경 시 `window.__appVolume`을 주입하고, 광고 스킵용
  ...
  광고 중 무음 처리는 `muted`라 볼륨 강제와 충돌하지 않는다. **인터벌만으로는 폴백 전환 순간에
  큰 소리가 샌다**(실측): 유튜브가 재생 시작 직후 자기 기억 볼륨(보통 100%)을 복원하는데 인터벌은
  최대 100ms 뒤에야 되돌리기 때문이다. 그래서 볼륨 적용을 `window.__applyVolume()`으로 빼
  ① `volumechange` 리스너에서 같은 이벤트 턴에 즉시 되돌리고, ② 호스트 오디오 게이트를 여는 신호인
  `__reportPlaying()` 직전에도 적용한다 — 소리를 여는 시점에는 항상 앱 볼륨이 맞춰져 있다.
...
- **창 디자인**: 카드 패널 없이 완전 투명 배경. 왼쪽에 정사각형(앨범 이미지 또는 미러 영상, 하단 그라디언트 위에
  ...
  `showLyrics` 토글은 가사 줄(`#lyrics-lines`)만 끈다 — 곡 정보(제목·아티스트)는 `showTrackInfo`가
  따로 관장해 가사를 꺼도 남는다(사용자 요청). 둘 다 꺼졌을 때만 오른쪽 열(`#lyrics-body`)을 접어
  왼쪽 사각형만 남긴다. `#lyrics-lines`는
  overflow:hidden — 가운데 정렬된 가사가 칸보다 길면 위로 넘쳐 곡 정보를 덮었다(z-index도 곡 정보가 위).
  줄마다 어두운 칩 배경을 따로 두르며(배경 불투명도 설정이 칩의 알파), 현재 블록은 크고 흰 글씨,
  이전/다음은 0.72배 크기의 옅은 글씨. 줄 간격·칩 여백은 촘촘하게(gap 4px, padding 1px 7px, line-height 1.28)
  — 한 화면에 더 많은 줄이 보이도록. 위아래 페이드 마스크·사각형 그림자·상태 문구(출처/언어)는
  사용자 요청으로 제거했다. 곡 정보 제목은 가사보다 1pt 크고 볼드. 앨범 아트 크기는
...
- **창 이동은 수동 드래그**: `-webkit-app-region: drag`는 `setIgnoreMouseEvents(true, {forward})` 히트박스
  ...
  **`-webkit-app-region: drag`는 플로팅 창 어디에도 남기면 안 된다** — 영상 미러 위의 투명 막에 남아 있던 것이
  영상 모드에서만 드래그가 안 되던 원인(누르는 순간 OS 창 이동으로 넘어가 pointerdown이 렌더러에 오지 않는다).
  Alt+4로 잠금을 풀 때는 **우선 마우스를 통째로 받게 해** 즉시 잡을 수 있게 하고, 커서가 상호작용 요소 밖이면
  렌더러가 첫 mousemove에서 통과 모드로 되돌린다(히트 알림 왕복을 기다리지 않는다). 히트 알림은 상태가 바뀔 때만 보낸다. 이때 `setPosition`이 아니라
  **`setBounds`에 폭·높이를 설정값으로 함께 넣는다** — Windows DPI 배율(125% 등)에서 프레임 없는 투명 창을
  `setPosition`으로 옮기면 크기가 조금씩 커진다(실측 신고). 드래그 중에는 커서가 박스 밖으로 살짝 나가도
  히트 상태를 놓지 않으며(`draggingWindow`), 왼쪽 열 전체(`#lyrics-side`)를 히트 영역으로 잡아 틈에서 놓치지 않는다.
  **`-webkit-app-region: drag`는 플로팅 창 어디에도 남기면 안 된다** — 영상 미러 위의 투명 막에 남아 있던 것이
  영상 모드에서만 드래그가 안 되던 원인(누르는 순간 OS 창 이동으로 넘어가 pointerdown이 렌더러에 오지 않는다).
  Alt+4로 잠금을 풀 때는 **우선 마우스를 통째로 받게 해** 즉시 잡을 수 있게 하고, 커서가 상호작용 요소 밖이면
  렌더러가 첫 mousemove에서 통과 모드로 되돌린다(히트 알림 왕복을 기다리지 않는다). 히트 알림은 상태가 바뀔 때만 보낸다.
...
- **한국어 곡명 표기**(`preferKoreanLabel`): 고른 가사의 제목이 원어면, ① 채택된 같은 곡 후보 중 한글 제목
  ...
  표기용 DB 제목은 `cleanLyricLabel`로 "[가사/해석/발음]" 같은 꼬리표를 뗀다. 플로팅 창은 유튜브 제목이
  한글인데 DB 제목이 원어면 한글 쪽을 유지한다(매칭 순간 원어로 바뀌던 문제).
- **Alt+1/2 꾹 누르기 가속**: 자동 반복 중에는 걸음을 ±4, 1.5초 이상이면 ±8로 키운다(첫 입력은 ±1 즉시). 볼륨 플래시는 갱신 중 애니메이션을 재시작하지 않는다 — 시작점(투명)에 묶여 안 보였다.
- **Alt+` 음소거 토글**: 렌더러 `toggleMute()`가 직전 볼륨을 기억해 0↔복원(없으면 50). 가사 창에는 🔇/🔊 플래시.
...
- **Alt+Q/E는 즉시 이전/다음 곡**(판정 대기 없음). 되감기/빨리 감기는 **Alt+W를 누른 채 Q/E를 꾹** 누르는
  코드. 전역 단축키는 키를 뗀 순간을 모르지만 누르고 있으면 OS 자동 반복으로 콜백이 계속 오므로: Alt+W 콜백이
  **최근 1초 안**에 왔으면 W를 누르고 있는 것으로 본다(W의 첫 자동 반복은 키 반복 지연 500ms~1초 뒤에야
  오므로 350ms로 잡았을 땐 그 사이에 누른 Q/E가 단발로 새어 "전혀 안 된다"고 느껴졌다) → 그때 오는 Q/E는
  슬라이드(100ms마다 ±2초); 시작되면 Q/E 자동 반복이 250ms 안에 이어지는 동안 계속. 코드가 아닌데 Q/E를
  붙잡고 있으면 자동 반복을 무시해 곡을 연달아 넘기지 않는다. Alt+W 자체의 재생/일시정지는 250ms 뒤 확정 —
  그 안에 Q/E가 따라오면 토글하지 않으며, 1초 안의 두 번째 Alt+W는 자동 반복으로 간주돼 무시된다.
...
- **제목/아티스트 파싱 규칙(사용자 신고 사례로 보강)**:
  ...
  - **가수 미상 항목은 채택하지 않는다** — `<알 수 없음>`뿐 아니라 가수 칸이 제목과 같거나 40자를 넘는 것
    (유튜브 제목을 통째로 올린 쓰레기 항목)도 포함.
  - **DB 항목 제목도 같은 정제를 거쳐 비교한다**(`candidateTitleVariants`): "Chenomio -フィクションです。feat.重音テト"
    처럼 "아티스트 - 제목 feat.X" 꼴로 올라온 항목은 아티스트 조각·feat·잡음을 뗀 핵심 제목으로도 맞춰 본다
    (수동 검색으론 나오는데 자동으론 못 찾던 사례). 같은 곡이 여럿이면 제목이 깔끔한 항목을 앞에 둔다.
